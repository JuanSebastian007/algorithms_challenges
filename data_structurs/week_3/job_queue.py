...
class JobQueue:
    def __init__(self, n_workers, jobs):
        # Creo la clase que me muestra la cola de procesamientos.
        self.n = n_workers
        self.jobs = jobs
        self.finish_time = []
        self.assigned_jobs = []
        # Incializo valores para los tiempos de procesamiento
        for i in range(self.n):
            self.finish_time.append([i, 0])

    def SiftDown(self, i):
        # Implementacion del siftdown
        min_index = i
        left = 2 * i + 1
        right = 2 * i + 2
        if left < self.n:
    ...
    def NextWorker(self, job):
        # Voy agregando la asignacion del worker segun la prioridad como orden de llegada.
